...
$executorInfo = User::findIdentity($model->executor);
?>

<tr>
    <td><a target="_blank" href="<?=Url::toRoute(['executor/incoming-task-details', 'taskId' => $model->id])?>"><?=Html::encode($model->task_header) ?></a></td>
    <td><?="Должна была быть завершена: {$taskItem->getTaskPlannedFinishTimeText()}<br/>"?></td>
    <td><?="Исполнитель: <b>{$executorInfo->name}</b><br/>" . Role::getPersonRole($executorInfo)?></td>
    <td><?php
